...
import {
  ...
  UpdateClinicAppointmentDto,
  QueryClinicAppointmentDto,
} from '../dto/clinic-appointment.dto';
import {
...
const TASK_TYPE = 'clinic_appointment';

@Controller('v1/api/organizations/:organizationId/clinic-appointments')
...
export class ClinicAppointmentsController {
  ...
  async findAll(
    ...
    @Query() query: QueryClinicAppointmentDto,
  ...
  async create(
    @Param('organizationId') organizationId: string,
    @Body() dto: CreateClinicAppointmentDto,
    @Request() req: RequestWithUser,
  ) {
    const userId = req.user?.userId ?? req.user?.sub;
    if (!userId) throw new UnauthorizedException('User ID not found');
    const data = await this.taskService.create(organizationId, TASK_TYPE, dto, userId);
    return SuccessHelper.createSuccessResponse(data, 'Appointment created');
  }
  ...
  async update(
    @Param('organizationId') organizationId: string,
    @Param('id') id: string,
    @Body() dto: UpdateClinicAppointmentDto,
    @Request() req: RequestWithUser,
  ) {
    const userId = req.user?.userId ?? req.user?.sub;
    if (!userId) throw new UnauthorizedException('User ID not found');
    const data = await this.taskService.update(organizationId, TASK_TYPE, id, dto, userId);
    return SuccessHelper.createSuccessResponse(data, 'Appointment updated');
  }
  ...
  async remove(
    ...
    return SuccessHelper.createSuccessResponse(null, 'Appointment removed');
  }
