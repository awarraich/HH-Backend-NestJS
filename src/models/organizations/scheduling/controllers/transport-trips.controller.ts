...
import {
  CreateTransportTripDto,
  UpdateTransportTripDto,
  QueryTransportTripDto,
} from '../dto/transport-trip.dto';
import {
...
const TASK_TYPE = 'transport_trip';

@Controller('v1/api/organizations/:organizationId/transport-trips')
...
export class TransportTripsController {
  ...
  async findAll(
    ...
    @Query() query: QueryTransportTripDto,
  ...
  async create(
    @Param('organizationId') organizationId: string,
    @Body() dto: CreateTransportTripDto,
    @Request() req: RequestWithUser,
  ) {
    const userId = req.user?.userId ?? req.user?.sub;
    if (!userId) throw new UnauthorizedException('User ID not found');
    const data = await this.taskService.create(organizationId, TASK_TYPE, dto, userId);
    return SuccessHelper.createSuccessResponse(data, 'Trip created');
  }
  ...
  async update(
    @Param('organizationId') organizationId: string,
    @Param('id') id: string,
    @Body() dto: UpdateTransportTripDto,
    @Request() req: RequestWithUser,
  ) {
    const userId = req.user?.userId ?? req.user?.sub;
    if (!userId) throw new UnauthorizedException('User ID not found');
    const data = await this.taskService.update(organizationId, TASK_TYPE, id, dto, userId);
    return SuccessHelper.createSuccessResponse(data, 'Trip updated');
  }
  ...
  async remove(
    ...
    return SuccessHelper.createSuccessResponse(null, 'Trip removed');
  }
