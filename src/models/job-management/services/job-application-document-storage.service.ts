...
export class JobApplicationDocumentStorageService {
  ...
  /**
   * Save a job application document (resume, cover letter, etc.) to local disk or S3.
   * Returns file_name (original) and file_url for use in submitted_fields.
   */
  async saveDocument(
    buffer: Buffer,
  ...
  private saveToLocal(
    ...
    const dir = path.join(this.storageConfig.path, JOB_APPLICATION_DOCUMENTS_SUBDIR);
    ...
    const fileUrl = `${baseUrl.replace(/\/$/, '')}/${apiPrefix}/job-management/job-applications/documents/files/${storedName}`;
  ...
  private async saveToS3(
    ...
    const key = `${JOB_APPLICATION_DOCUMENTS_SUBDIR}/${storedName}`;
    await client.send(
  ...
  getLocalFilePath(storedFilename: string): string | null {
    ...
    const filePath = path.join(
      this.storageConfig.path,
      JOB_APPLICATION_DOCUMENTS_SUBDIR,
      storedFilename,
    );
