...
export class EmailService implements OnModuleInit {
  ...
  /**
   * Send a pre-built HTML notification (e.g. the in-service reminder
   * generated by InserviceNotificationService). The caller is responsible
   * for constructing the subject, HTML, and text bodies; this just hands
   * them to the configured SMTP transport with the standard from-address
   * and inline logo. Throws when SMTP credentials are missing so the
   * caller can decide whether to surface the failure.
   */
  async sendCustomNotificationEmail(args: {
    toEmail: string;
    subject: string;
    html: string;
    text: string;
    ...
    if (!auth.user || !auth.pass) {
      throw new Error(
        'Email service not configured. Set EMAIL_USER and EMAIL_PASSWORD to send notification emails.',
      );
    }
    const mailOptions = {
      from: `"${this.emailConfigService.fromName}" <${this.emailConfigService.from}>`,
    ...
    const info = await this.transporter.sendMail(mailOptions);
    this.logger.log(
      `Notification email sent to: ${this.maskEmail(args.toEmail)}. MessageId: ${info.messageId}`,
    );
  }
